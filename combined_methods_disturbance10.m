...
%% Friction model function (Stribeck + viscous + static friction)
...
if run_smc
    ...
    for i = 2:N
        ...
        % --- Error + sliding surface (with terminal term) ---
        ...
        % --- Control law (terminal / super-twisting / normalization) ---
...
inset1 = axes('Position',[0.41,0.76,0.08,0.08]); % Control position
box on; hold on;
...
xlabel('Time [s]', 'FontSize', fontsize);
set(gca, 'FontSize', fontsize);
ylim_max = max([abs(pid_Fm), abs(smc_Fm), abs(rbf_Fm)], [], 'all'); % Max absolute value among three methods
ylim([-ylim_max, ylim_max]); 
box on;

axes2 = gca;
rectangle('Position', [3.2, -12, 0.2, 3.5], 'EdgeColor', 'r', 'LineWidth', 1.2);
inset2 = axes('Position',[0.35,0.21,0.08,0.08]); % Control position
box on; hold on;
plot(t, pid_Fm, 'b', 'LineWidth', 1);
plot(t, smc_Fm, 'r--', 'LineWidth', 1);
plot(t, rbf_Fm, 'Color', [0.466 0.674 0.188], 'LineStyle', '-.', 'LineWidth', 1);
xlim([3.2 3.4]); ylim([-12 -8.5]);
set(gca, 'FontSize', 8);

...
smooth_sign = @(x) tanh(10 * x);  % Smoother alternative to sign, sacrificing precision for smoothness
...
%% ================= SMC Controller =================
if run_smc
    % --- Terminal Super-Twisting Parameters ---
    lambda = 20;
    rho = 0.8;
    alpha = 5;
    phi = 0.5;
    ...
    for i = 2:N
        ...
        % --- Control Law (with Terminal/Normalized ST-SMC) ---
...
xlabel('Time [s]', 'FontSize', fontsize);
set(gca, 'FontSize', fontsize);
ylim_max = max([abs(pid_Fm), abs(smc_Fm), abs(rbf_Fm)], [], 'all');
ylim([-ylim_max, ylim_max]); 
box on;
axes2 = gca;
rectangle('Position', [3.1, -22, 0.4,4], 'EdgeColor', 'r', 'LineWidth', 1.2);
inset2 = axes('Position',[0.35,0.21,0.08,0.08]);
box on; hold on;
plot(t, pid_Fm, 'b', 'LineWidth', 1);
plot(t, smc_Fm, 'r--', 'LineWidth', 1);
plot(t, rbf_Fm, 'Color', [0.466 0.674 0.188], 'LineStyle', '-.', 'LineWidth', 1);
xlim([3.1 3.5]); ylim([-22 -18]);
set(gca, 'FontSize', 8);

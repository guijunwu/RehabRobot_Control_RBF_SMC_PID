...
ylim([-ylim_max, ylim_max]); % make y-axis symmetric
...
if run_pid
    ...
    fprintf('Root Mean Square Error (RMSE): %.5f m/s\n', metric_pid.rmse);
    fprintf('Maximum Absolute Error       : %.5f m/s\n', metric_pid.maxerr);
...
if run_smc
    fprintf('\n== Super-Twisting SMC Performance Metrics ==\n');
    fprintf('Root Mean Square Error (RMSE): %.5f m/s\n', metric_smc.rmse);
    fprintf('Maximum Absolute Error       : %.5f m/s\n', metric_smc.maxerr);
...
if run_rbf
    fprintf('\n== RBF Neural Network Control Performance Metrics ==\n');
    fprintf('Root Mean Square Error (RMSE): %.5f m/s\n', metric_rbf.rmse);
    fprintf('Maximum Absolute Error       : %.5f m/s\n', metric_rbf.maxerr);
...
function settle_time = calc_settling_time(err, t, threshold, duration)
    ...
    settle_time = NaN;  % return NaN if condition is never satisfied
end
